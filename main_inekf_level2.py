#!/usr/bin/env python3
"""Train Level 2 InEKF (heteroscedastic R_t) MapFormer."""
...
def main():
    ...
    parser.add_argument("--output-dir", type=str, default="figures_inekf_level2")
    ...
    model = MapFormerWM_Level2InEKF(
        vocab_size=env.unified_vocab_size,
        d_model=128, n_heads=2, n_layers=1, grid_size=64,
    )
    print(f"Level 2 InEKF: {sum(p.numel() for p in model.parameters()):,} params")
    ...
    ckpt = out / "MapFormer_WM_Level2InEKF.pt"
    torch.save({
