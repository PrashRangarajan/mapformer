...
LOG="$REPO/family_tree_d7.log"; : > "$LOG"; echo "start $(date)" >> "$LOG"
python3 -u -m mapformer.validate_family_tree --depth 7 --n-obs 8 \
  --out "$REPO/FAMILY_TREE_D7_GATES.md" >> "$LOG" 2>&1
OUT="$REPO/runs/family_tree_d7"
...
run(){ local j=$1; IFS=: read -r v s <<<"$j"
  ...
  python3 -u -m mapformer.train_family_tree --variant "$v" --seed "$s" \
    --epochs 100 --n-batches 48 --batch-size 16 --depth 7 --n-obs 8 \
    --n-steps 128 --eval-steps 128 256 --n-layers 2 --device cuda:0 \
    --output-dir "$o" >> "$LOG" 2>&1; }
for j in "${JOBS[@]}"; do run "$j"; done

python3 -u - "$OUT" "$REPO/FAMILY_TREE_D7_RESULTS.md" >> "$LOG" 2>&1 <<'PY'
...
ln = ["# Family-tree (non-commutative relational structure) — results", "",
      ...
      "n=3 seeds. Trained at 128 steps, also evaluated at 256 (OOD length).", "",
      ...
      "| variant | n_steps=128 (train) | n_steps=256 (OOD) |", "|---|---|---|"]
for k, lab in V:
    acc = {L: [] for L in LS}
    for s in (0,1,2):
...
dst.write_text("\n".join(ln)+"\n"); print("\n".join(ln))
PY
cd "$REPO"; git add FAMILY_TREE_D7_RESULTS.md FAMILY_TREE_D7_GATES.md FAMILY_TREE_GATES.json run_family_tree.sh 2>/dev/null
...
time. Floor is the hub baseline 0.146, not chance 0.125. Auto-committed;
interpretation pending."; git push origin main >> "$LOG" 2>&1; }
echo "$(date +%H:%M) DONE" >> "$LOG"; touch "$REPO/.family_tree_d7_done"
