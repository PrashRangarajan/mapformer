#!/usr/bin/env python3
"""Level 1.5 on MapFormer-EM backbone: multi-seed orchestrator.

Runs 3 configs x 3 seeds = 9 training runs for Level15EM (Level 1.5 InEKF
correction on top of the stronger MapFormer-EM backbone). This lets us
compare:
    Vanilla-WM / Vanilla-EM / Level15-WM / Level15-EM
in every regime (clean / noise / lm200), answering:
  - Does the stronger EM backbone make the correction redundant?
  - Does the correction still pay for itself on top of EM?

Usage:
  nohup python3 -u -m mapformer.orchestrator_level15_em \
      > mapformer/orchestrator_level15_em.log 2>&1 &
"""
...
for cfg in [("clean", 0.0, 0), ("noise", 0.10, 0), ("lm200", 0.10, 200)]:
    ...
    for seed in [0, 1, 2]:
        out_dir = RUNS_DIR / f"Level15EM_{tag}" / f"seed{seed}"
        EXPERIMENTS.append({
            "variant": "Level15EM",
            "seed": seed,
            "n_landmarks": n_lm,
            ...
            "ckpt_path": out_dir / "Level15EM.pt",
            "n_epochs": 50,
            "n_batches": 156,
            "tag": tag,
        })
...
print(f"[{datetime.now()}] Level15EM orchestrator starting")
...
print(f"[{datetime.now()}] Level15EM orchestrator done.")
