#!/usr/bin/env bash
# EM vs WM x rank on allocentric DoorKey-16x16. See MINIGRID_EM_PREREG.md.
# NO --fast-attn: invalid for MapEM (Hadamard attention is not SDPA), and every arm
# in a batch must share the setting.
set -u
REPO=/home/prashr/mapformer; cd /home/prashr
OUT=$REPO/runs/minigrid_em; mkdir -p "$OUT" "$OUT/logs"

# Build the allocentric buffer ONCE, serially: MiniWorld/MiniGrid make an EGL
# context per worker, and parallel builders pinned a 4090 before any model loaded.
echo "building allocentric buffer $(date +%H:%M)"
python3 -c "
from mapformer.minigrid_env import MiniGridWorld_Cached as C
e=C(env_name='MiniGrid-DoorKey-16x16-v0', tokenization='obj_color',
    buffer_size=25000, allocentric=True); e.generate_batch(2,128)" || exit 1
echo "buffer ready $(date +%H:%M)"

ARMS="RoPE Vanilla Vanilla_r4 VanillaEM VanillaEM_r4"
...
echo "missing=$miss"; [ "$miss" -eq 0 ] && touch "$REPO/.minigrid_em_done"
echo "finished $(date)"
