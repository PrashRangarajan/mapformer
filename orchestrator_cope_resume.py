#!/usr/bin/env python3
"""CoPE resume orchestrator — picks up the 5 CoPE runs we deferred.

Already done: CoPE_clean seed0/1/2, CoPE_noise seed0.
Remaining:    CoPE_noise seed1/2, CoPE_lm200 seed0/1/2.

Each CoPE run on our setup takes ~8 hours (vs LSTM's 8 minutes), due
to CoPE's O(L^2) per-position cumulative-sum attention. 5 remaining
runs on 2 GPUs ≈ 20 hours of wall time. Launched after MambaLike +
finalize to avoid blocking the higher-priority result.

Usage:
  nohup python3 -u -m mapformer.orchestrator_cope_resume \
      > mapformer/orchestrator_cope_resume.log 2>&1 &
"""
...
for cfg in [("clean", 0.0, 0), ("noise", 0.10, 0), ("lm200", 0.10, 200)]:
    ...
    for seed in [0, 1, 2]:
        out_dir = RUNS_DIR / f"CoPE_{tag}" / f"seed{seed}"
        EXPERIMENTS.append({
            "variant": "CoPE", "seed": seed, "n_landmarks": n_lm,
            "p_action_noise": p_noise, "output_dir": str(out_dir),
            "ckpt_path": out_dir / "CoPE.pt",
            "n_epochs": 50, "n_batches": 156, "tag": tag,
        })

REMAINING = [e for e in EXPERIMENTS if not e["ckpt_path"].exists()]
print(f"[{datetime.now()}] CoPE resume orchestrator: {len(REMAINING)}/{len(EXPERIMENTS)} to run")
...
print(f"[{datetime.now()}] CoPE resume orchestrator done.")
