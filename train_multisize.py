"""Train any variant on multi-size, multi-environment data.

Each batch trajectory is sampled from a random env (random size + random
instance). Eval on held-out envs at each size separately so we can see
per-size performance.

Tests whether the cognitive-map machinery learns scale-invariant or
scale-conditional strategies across genuinely different grid sizes.
"""
...
from mapformer.environment_multisize import MultiSizeMultiEnvGridWorld
...
def evaluate_size(model, world, size, T, n_trials, device, seed=2000, train_envs=False):
...
def main():
    ...
    ap.add_argument("--n-envs-per-size", type=int, default=20)
    ...
    world = MultiSizeMultiEnvGridWorld(
        sizes=tuple(args.sizes),
        n_envs_per_size=args.n_envs_per_size,
        n_test_envs_per_size=args.n_envs_per_size,
        n_obs_types=16, p_empty=0.5,
        n_landmarks=args.n_landmarks,
        seed=args.seed,
    )
    cls = VARIANT_MAP[args.variant]
    # Use grid_size=64 as "reference" but model is grid-size-agnostic
    ...
    total_train_envs = len(args.sizes) * args.n_envs_per_size
    print(f"{args.variant} seed={args.seed} sizes={args.sizes} "
          f"n_train={total_train_envs} (={args.n_envs_per_size}/size) lm={args.n_landmarks}")
    print(f"params={sum(p.numel() for p in model.parameters()):,}")

    losses = train(model, world, args.epochs, args.lr, args.batch_size,
                   args.n_steps, args.n_batches, args.device)

    # Eval per size on held-out envs
    eval_results = {}
    for size in args.sizes:
        print(f"\n--- Eval at size {size} ---")
        acc_128, nll_128 = evaluate_size(model, world, size, args.n_steps, 100,
                                          args.device, seed=2000, train_envs=False)
        acc_512, nll_512 = evaluate_size(model, world, size, args.n_steps * 4, 50,
                                          args.device, seed=2000, train_envs=False)
        eval_results[size] = {
            "held_T128_acc": acc_128, "held_T128_nll": nll_128,
            "held_T512_acc": acc_512, "held_T512_nll": nll_512,
        }
        print(f"  size {size}: T=128 acc={acc_128:.3f} nll={nll_128:.3f}")
        print(f"  size {size}: T=512 acc={acc_512:.3f} nll={nll_512:.3f}")

    ckpt = out / f"{args.variant}_multisize.pt"
    torch.save({
